...
export default {
    ...
    getTodayForcast: async (nx, ny) => {
        try {
            const data = await axios.get('/api' + `${config.weatherUrls}${config.TodayForcast}ServiceKey=${config.keys.weatherEncodingKey}&pageNo=1&numOfRows=865&dataType=JSON&base_date=${Number(hour) < 3 ? dayjs(today).subtract(1, "day").format("YYYYMMDD") : today}&base_time=${Number(hour) < 3 ? "2300" : "0200"}&nx=${nx}&ny=${ny}`);
            // console.log(data);
            return { data: data };
        } catch (err) {
            throw new Error(err);
